...
<body x-data="{ mobileMenuOpen: false }" class="h-full bg-white text-[#111111] font-sans antialiased flex flex-col selection:bg-[#e8dcd2] selection:text-[#111111]">
    ...
    @php
        $recentPurchases = [];
        try {
            $recentOrders = \Lunar\Models\Order::whereNotNull('placed_at')
                ->latest('placed_at')
                ->limit(8)
                ->with(['lines.purchasable.product', 'shippingAddress'])
                ->get();
                
            foreach ($recentOrders as $order) {
                if ($order->shippingAddress && $order->lines->isNotEmpty()) {
                    $line = $order->lines->first();
                    $pName = $line->purchasable?->product?->attr('name') ?? 'a product';
                    $recentPurchases[] = [
        ...
        if (empty($recentPurchases)) {
            $names = ['Fatima', 'Ayesha', 'Zainab', 'Sana', 'Amna', 'Hamza', 'Arsalan', 'Ali', 'Bilal', 'Hassan', 'Mariam', 'Zahra', 'Kiran', 'Osman'];
            $cities = ['Karachi', 'Lahore', 'Islamabad', 'Faisalabad', 'Rawalpindi', 'Multan', 'Peshawar', 'Sialkot', 'Gujranwala', 'Hyderabad'];
            
            try {
                $products = \Lunar\Models\Product::all()
                    ->map(fn($p) => $p->attr('name'))
                    ->filter()
                    ->values()
                    ->toArray();
            } catch (\Exception $e) {
            ...
            if (empty($products)) {
                $products = [
                    'KELVS Whitening Serum',
                    'KELVS Vitamin C Serum',
                    'KELVS Anti Aging Serum',
                    'KELVS Hydration Serum',
                    'KELVS Gentle Cleanser',
                    'KELVS Toning Solution',
                ];
            }
    ...
    <!-- Social Proof Recent Purchases Toast Styles -->
    <style>
        .recent-buyer-toast {
            bottom: 1.5rem !important; /* Default bottom: 24px */
        }
    </style>

    <!-- Social Proof Recent Purchases Toast -->
    <div x-data="{
            purchases: @js($recentPurchases),
            currentIndex: 0,
            show: false,
            init() {
                if (this.purchases.length === 0) return;
                setTimeout(() => {
                    this.triggerNextToast();
                }, 3000); // Wait 3 seconds before first popup
            },
            triggerNextToast() {
                this.show = true;
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            this.triggerNextToast();
                        }, 12000 + Math.random() * 6000); // Trigger next toast every 12-18 seconds (natural timing)
                    }, 600); // Wait for transition out to finish
                }, 5000); // Display for 5 seconds (easier to read, not too long)
            }
         }"
         x-show="show"
         ...
         class="fixed left-6 z-[999] recent-buyer-toast w-[calc(100%-48px)] sm:w-80 max-w-sm bg-white border border-gray-200 rounded-xl shadow-xl p-4 flex items-center gap-4"
         style="display: none;"
    >
        <div class="h-10 w-10 rounded-full bg-emerald-50 border border-emerald-100 flex items-center justify-center shrink-0">
        ...
        <div class="text-xs text-gray-500">
            <p class="font-bold text-[#111111] leading-snug">
                <span x-text="purchases[currentIndex].name"></span> from <span x-text="purchases[currentIndex].city"></span>
            </p>
            <p class="mt-0.5 text-gray-600">
                Purchased <span x-text="purchases[currentIndex].product" class="font-bold text-emerald-700"></span>
            </p>
            <p class="mt-1 text-[9px] text-gray-400 font-medium" x-text="purchases[currentIndex].time"></p>
        </div>
    </div>
</body>
</html>
